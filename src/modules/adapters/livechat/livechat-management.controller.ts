...
export class LivechatManagementController {
    ...
    async sendFile(@Body() body: { chatId: string; fileUrl: string; contentType: string; fileName: string }) {
        return this.livechatApi.sendFile(body.chatId, body.fileUrl, body.contentType, body.fileName);
    }
    ...
    async transferChat(
        ...
        return this.livechatApi.transferChat(chatId, body.targetAgentIds, body.force);
    }
    ...
    @ApiBody({
        schema: {
            type: 'object',
            properties: {
                url: { type: 'string', example: 'https://YOUR_NGROK_URL.ngrok-free.app/livechat/webhook/incoming' },
                action: { type: 'string', example: 'incoming_chat_thread' },
                secretKey: { type: 'string', example: 'my_secret_key' },
                ownerClientId: { type: 'string', example: 'YOUR_LIVECHAT_APP_CLIENT_ID' },
                type: { type: 'string', example: 'license', enum: ['license', 'bot'] },
            },
        },
    })
    async registerWebhook(
        @Body() body: { url: string; action: string; secretKey: string; ownerClientId: string; type?: 'license' | 'bot' },
    ) {
        if (!body?.action) {
            return { success: false, error: 'Thiếu action hoặc body trống' };
        }
        if (!body.ownerClientId) {
            return { success: false, error: 'Thiếu ownerClientId (Client ID của App trên LiveChat)' };
        }
