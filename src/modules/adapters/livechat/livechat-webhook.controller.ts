...
export class LivechatWebhookController {
  ...
  @ApiOperation({
    summary:
      'Endpoint nhận webhook từ LiveChat (cấu hình URL này trên LiveChat)',
  })
  async receiveWebhook(@Body() payload: LivechatWebhookPayload) {
    this.logger.log(
      `📨 LiveChat Webhook received: ${payload?.action || 'unknown'}`,
    );
    this.logger.debug(`Payload: ${JSON.stringify(payload).substring(0, 500)}`);

    // Parse webhook → IStandardMessage
    const standardMessage =
      await this.livechatAdapter.parseIncomingWebhook(payload);
